...
export default function ProjectGroupList(props) {
    ...
    const onGridReady = (params) => {
        ...
        axios.get(process.env.REACT_APP_APIURL + '/api-web/searchProjectGroup').then((response) => {
    ...
    const searchData = () => {
        axios.get(process.env.REACT_APP_APIURL + '/api-web/searchProjectGroup', { params: { search: searchRef.current.value,status: statusRef.current.value } }).then((response) => {
    ...
    const resetsearch = () => {
        ...
        axios.get(process.env.REACT_APP_APIURL + '/api-web/searchProjectGroup')
            .then((response) => {
                setRowData(response.data);
            })
    };
    const deleteSingleRow = params => {
        axios.post(process.env.REACT_APP_APIURL + '/api-web/deleteProjectGroup', null, { params: { id: params.data.project_group_id } })
            .then((response) => {
    ...
    const deleteRowData = () => {
        ...
        selectedNodes.map((row, i) => {
            ...
            axios.post(process.env.REACT_APP_APIURL + '/api-web/deleteProjectGroup', null, { params: { id: row.data.project_group_id } })
                .then((response) => {
                    if (response.data.success === false) {
                        Swal.fire({
                            icon: 'error',
    ...
    const handleEdit = params => {
        navigate(`edit`, {state:{id:params.data.project_group_id}})
        console.log(params.data.project_group_id);
    };
    ...
    return (
        <section className="row custom-admin-input" >
            <AdminBanner title={"การจัดการโครงการ"} path={<>
                ...
                <Link id="profile-btn" to={{ pathname: "/admin/list" }}>{"จัดการหมวดหมู่"}</Link>
                <span><i className="fas fa-angle-right"></i></span>
                <GrFormNext/>
                <Link className="active" to={{ pathname: "/list" }}>{"รายการหมวดหมู่"}</Link></>}
            />
                    ...
                    <div className="col-12">
                        <div className="ag-theme-alpine" style={{ height: 900 }}>
                            ...
                            >
                                ...
                                <AgGridColumn field='project_group_name' headerName="ชื่อรายการ" cellRenderer={NameRenderer} minWidth={200} />
                                ...
                                <AgGridColumn field="project_group_id" headerName="จัดการ" cellRenderer="rowTool" minWidth={130} sortable={false} filter={false} resizable={false} />
                            </AgGridReact>
