...
export default function EditProject(props) {
    ...
    const UserName = currentUser.FirstnameTH+" "+currentUser.LastnameTH;
    ...
    useEffect(() => {
        console.log('id = '+ location.state.id )
        axios.get(process.env.REACT_APP_APIURL + '/api-web/searchProjectGroup', { params: { id:location.state.id }}).then((response) => {
            console.log(response.data)
            console.log(response.data[0].project_group_name)
            setNameTH(response.data[0].project_group_name)
            setNameEN(response.data[0].project_group_name_eng)
            setStatus(response.data[0].is_active)
        })
        
    }, [location.state.id]); /* <-- add this for run once*/
    ...
    const saveData = (open) => {
        if (handleValidation()) {
            ...
            axios.post(process.env.REACT_APP_APIURL + '/api-web/editProjectGroup', null, { params: { id:location.state.id ,titleth: nameth, titleen: nameen, status: status, UserID: currentUser.UserID ,createby: UserName } })
                .then((response) => {
                    let timerInterval;
                    swal.fire({
    ...
    const cancel = () => {
        navigate('/admin/projectGroup',{replace:true})
    };
    return (
        <section className="row custom-admin-input" >
            <AdminBanner title={"สร้างหมวดหมู่"} path={<>
                <span><i className="fas fa-angle-right"></i></span>
                <Link id="profile-btn" to={{ pathname: "/admin/list" }}>{"จัดการหมวดหมู่"}</Link>
                <span><i className="fas fa-angle-right"></i></span>
                <GrFormNext/>
                <Link id="profile-btn" className="active" to={{ pathname: "/admin/activity/NewActivityCategory" }}>{"แก้ไขหมวดหมู่"}</Link>
            </>}
            />
                        ...
                        <div className="tab-content pl-4 pr-4" id="myTabContent">
                            ...
                            <div className={activeTab === 1 ? "tab-pane fade show active" : "tab-pane"} id="tab1" role="tabpanel" aria-labelledby="tab1-tab">
                                <div className="form-group">
                                    <label htmlFor="thname" className="required">ชื่อหมวดหมู่ (ภาษาไทย)</label>
