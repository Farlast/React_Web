...
export default function NewProject(props) {
    ...
    const saveData = (open) => {
        if (handleValidation()) {
            console.log({params: { "titleth": nameth, "titleen": nameen, 'status': status, 'createby': UserName }});
            axios.post(process.env.REACT_APP_APIURL + '/api-web/newProjectGroup', null, { params: { titleth: nameth, titleen: nameen, status: status, createbyid: currentUser.UserID ,createby: UserName } })
                .then((response) => {
                    let timerInterval;
                    swal.fire({
    ...
    return (
        <section className="row custom-admin-input" >
            <AdminBanner title={"สร้างหมวดหมู่"} path={<>
                ...
                <span><i className="fas fa-angle-right"></i></span>
                <GrFormNext/>
                <Link id="profile-btn" className="active" to={{ pathname: "/admin/activity/NewActivityCategory" }}>{"สร้างหมวดหมู่"}</Link>
            </>}
            />
