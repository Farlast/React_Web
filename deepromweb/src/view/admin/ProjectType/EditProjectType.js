...
export default function EditProject(props) {
    ...
    useEffect(() => {
        console.log('id = '+ location.state.id )
        axios.get(process.env.REACT_APP_APIURL + '/api-web/searchProjecttype', { params: { id:location.state.id }}).then((response) => {
            console.log(response.data)
            console.log(response.data[0].project_type_name)
            setNameTH(response.data[0].project_type_name)
            setNameEN(response.data[0].project_type_name_eng)
    ...
    const saveData = (open) => {
        if (handleValidation()) {
            ...
            axios.post(process.env.REACT_APP_APIURL + '/api-web/editProjectType', null, { params: { id:location.state.id ,titleth: nameth, titleen: nameen, status: status, UserID: currentUser.UserID } })
                .then((response) => {
                    let timerInterval;
                    swal.fire({
    ...
    return (
        <section className="row custom-admin-input" >
            <AdminBanner title={"การจัดการโครงการ"} path={<>
                ...
                <span><i className="fas fa-angle-right"></i></span>
                <GrFormNext/>
                <Link id="profile-btn" className="active" to={{ pathname: "/admin/projectType/edit" }}>{"แก้ไขประเภท"}</Link>
            </>}
            />
